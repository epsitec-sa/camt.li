var T = {
  undefinedFormat: "Diese Datei wurde nicht erkannt.",
  camt53V2: "camt.053 (V2) Datei",
  camt54V2: "camt.054 (V2) Datei",
  camt53V4: "camt.053 (V4) Datei",
  camt54V4: "camt.054 (V4) Datei",
  camt53V8: "camt.053 (V8) Datei",
  camt54V8: "camt.054 (V8) Datei",
  fileName: "Datei",
  creationDate: "Erstellt am",
  customerAccount: "Kundenkonto",
  transactions: "Transaktionen",
  transactionsNo: "Anzahl Transaktionen",
  incomesNo: "Anzahl ESR Einzahlungen",
  origAmount: "Betrag (Fremdwährung)",
  exchangeRate: "Wechselkurs",
  openBalance: "Eröffnungssaldo",
  closeBalance: "Abschlusssaldo",
  dateValuta: "Valuta",
  dateBooking: "Buchungsdatum",
  totalCharge: "Gesamtkosten",
  total: "Gesamtbetrag",
  movement: "Bewegung",
  debtor: "Debitor",
  finInstitute: "Finanzinstitut",
  reference: "Referenz",
  charges: "Kosten",
  amount: "Betrag",
  downloadV11: "V11 Download",
  paymentMode: "Zahlungsart",
  cdpt: "Postbüro",
  autt: "Elektronischer Transfer",
  dmct: "Zahlungsauftrag",
  atxn: "EuroSIC",
  vcom: "Ohne Herkunft",
  cajt: "Rückleitung",
  credit: "Haben",
  debt: "Soll",
  parameters: "Parameter",
  type3: "Typ 3",
  type4: "Typ 4",
  withCrLf: "Mit CR+LF",
  withoutCrLf: "Ohne CR+LF",
  errorMessageTitleMissingInformation:
    "Eine oder mehrere Transaktionen wurden nicht konvertiert; es fehlen Informationen",
  errorMessageTitleNoBvrs: "Die übermittelte(n) Datei(en) enthalten keine Einzahlungen",
  errorMessageUnknown: "Unerwarteter Fehler (mehr dazu im Logfenster des Browsers)",
  errorMessageMissingRefs: "Fehlender Zahlungstyp (z.B. Zahlung auf eigenes Konto, Rückerstattung, usw.)",
  errorMessageQRIbanDetected:
    "Diese Datei enthält Referenzen, die mit einer QR-IBAN verknüpft sind. Sie kann nicht in das V11-Format konvertiert werden. Verwenden Sie für Ihre Rechnungen und Zahlungseingänge lieber Crésus.",
};
