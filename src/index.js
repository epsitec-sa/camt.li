...
function getDetailsSummary(details, clientBvrNumber, bvrsInfo) {
  ...
  const chargesAmount = formatAmount(
    _(() => details.Chrgs[0].TtlChrgsAndTaxAmt[0]._) || _(() => details.Chrgs[0].Rcrd[0].Amt[0]._)
  );
  ...
  const debtorDetails = debtorBank1.length
    ? debtorBank1 + (debtorBank2.length ? "<br/>" + formatIBAN(debtorBank2) : "")
    : debtorBank2.length
    ? formatIBAN(debtorBank2)
    : "-";
...
function getEntrySummary(entry, bvrsInfo) {
  ...
  const chargesAmount = formatAmount(
    _(() => entry.Chrgs[0].TtlChrgsAndTaxAmt[0]._) || _(() => entry.Chrgs[0].Rcrd[0].Amt[0]._)
  );
  const chargesCurrency =
    _(() => entry.Chrgs[0].TtlChrgsAndTaxAmt[0].$.Ccy) || _(() => entry.Chrgs[0].Rcrd[0].Amt[0].$.Ccy);
...
function hideErrorBox() {
  ...
  document.getElementById("messageTitleMissingInformation").style.display = "none";
  ...
  document.getElementById("messageQRIbanDetected").style.display = "none";
}
              ...
              <table>
                <tr>
                  <td class="typeButton" style="width: 20%;">
                      <input type="radio" name="type" value="3-100" id="type-3-100" ${
                        typeChoice === "3-100" ? "checked" : ""
                      }>
                      <label for="type-3-100">${T.type3} / 100</label>
                  </td>
                  <td class="typeButton" style="width: 20%;">
                      <input type="radio" name="type" value="3-128" id="type-3-128" ${
                        typeChoice === "3-128" ? "checked" : ""
                      }>
                  ...
                  <td class="typeButton" style="width: 20%;">
                      <input type="radio" name="crlf" value="off" id="crlf-off" ${
                        crLfChoice === "off" ? "checked" : ""
                      }>
      ...
      getXmlReport(file.name, e.target.result, (err, html, xml) => {
        ...
        try {
          ...
          typeChoices.forEach((choice) => {
            choice.addEventListener("click", () => writeV11Type(choice.value), false);
          });

          const crLfChoices = Array.from(document.getElementsByName("crlf"));
          crLfChoices.forEach((choice) => {
            choice.addEventListener("click", () => writeV11CrLf(choice.value), false);
          });
        } catch (ex) {
          console.log(ex);
        }
